...
function displayBookingDetails () {
    ...
    var bookings = JSON.parse(localStorage.getItem('carbookingsNew')) || [];
    var cartotalPrices = JSON.parse(localStorage.getItem('cartotalPrices')) || [];

    var tbody = $('#cardetailsTable tbody');
    ...
    function saveRow(row) {
        ...
        cartotalPrices[index] = cells.eq(6).find('input').val();

        // Update data in localStorage
        localStorage.setItem('carbookingsNew', JSON.stringify(bookings));
        localStorage.setItem('cartotalPrices', JSON.stringify(cartotalPrices));
...
$(document).ready(function () {
    // Retrieve data from local storage
    const formDataArray = JSON.parse(localStorage.getItem('carformDataArray')) || [];
    const placesArray = JSON.parse(localStorage.getItem('carplacesArray')) || [];
    const bookingsNew2 = JSON.parse(localStorage.getItem('carbookingsNew')) || [];

    // Reference to the table body
    const tbody2 = $('#carcontactdetailsTable tbody');
    ...
    function populateContactDetailsTable() {
        ...
        formDataArray.forEach((formData, index) => {
            const row = $('<tr>');
            row.append('<td>' + (index + 1) + '</td>');
            row.append('<td>' + bookingsNew2[index].id + '</td>');
...
function saveRow1(row) {
    ...
    // Update data in localStorage
    localStorage.setItem('carformDataArray', JSON.stringify(formDataArray));
    ...
    function deleteRow1(row) {
        ...
        // Update localStorage with the modified data
        localStorage.setItem('carformDataArray', JSON.stringify(formDataArray));
        localStorage.setItem('carplacesArray', JSON.stringify(placesArray));
        localStorage.setItem('carbookingsNew', JSON.stringify(bookingsNew2));
